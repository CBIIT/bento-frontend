  ...
  return () => {
    ...
    return client
      .query({
        ...
        variables: { ...allFilters(), ...getState().autoCompleteSelection },
      })
  ...
  const filter = Object.entries(currentAllActiveFilters).reduce((acc, [key, val]) => {
    if (data[0].datafield === key) {
      return data[0].isChecked
        ? {
          ...acc,
          [key]: [...currentAllActiveFilters[key], ...[data[0].name]],
        }
        : {
          ...acc,
          [key]: currentAllActiveFilters[key].filter((item) => item !== data[0].name),
        };
    }
...
export function addAutoComplete({ value, type }) {
