...
export  function getColumns(tableConfig, classes, data, externalLinkIcon){
              ...
              : (
                <div>
                  ...
                  {column.dataFromRoot ? data[column.dataField]
                   ...
                   <Link className={classes.link} to={(location) => ({ ...location, pathname: '/cases' })} onClick={() => redirectTo('TAILORx')}>{value}</Link>
                  : `${value}`
                  )}
                  {' '}
...
export const getDefaultCustomFooter = (count, page, rowsPerPage, changeRowsPerPage, changePage, classes) => {
  if(count>=11){
    return <TableFooter>
        <TableRow>
              <TablePagination
...
export function getOptions(table, classes, customFooter, onRowSelectionChange,isRowSelectable) {
  return {
    ...
    downloadOptions: {
      filename:
        table.downloadOptions && table.downloadOptions.downloadFileName
          ? table.downloadOptions.downloadFileName.concat(dateTimeStamp()).concat('.csv') : 'Bento_files_download.csv',
    filterOptions: {
    ...
    customFooter: (count, page, rowsPerPage, changeRowsPerPage, changePage) => customFooter ? customFooter(count, page, rowsPerPage, changeRowsPerPage, changePage, classes) : getDefaultCustomFooter(count, page, rowsPerPage, changeRowsPerPage, changePage, classes),
    isRowSelectable: (dataIndex) => isRowSelectable ? isRowSelectable(dataIndex) : null ,
    onRowSelectionChange: (curr, allRowsSelected) => onRowSelectionChange ? onRowSelectionChange(curr, allRowsSelected):null ,
  };
}
